...
function wpdf_enqueue_public_scripts()
{
    if (wpdf_current_user_role()) {
         ...
         */
        global $users;
        wp_enqueue_script('wpdf-main-js', plugin_dir_url(__FILE__) . 'inc/js/wpdf-script.min.js', array( 'jquery' ), '');
        wp_localize_script('wpdf-main-js', 'params', $users);
        
        // Enqueue datatables JS file
        wp_enqueue_script('datatables-js', plugin_dir_url(__FILE__) . 'lib/mdb/js/datatables.min.js', array( 'jquery' ));
    }
}
